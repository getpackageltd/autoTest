...
public class TestNGCustomReportListener implements IReporter{
    ...
    private void resultSummary(ISuite suite, IResultMap tests, String testname,
        ...
        if (tests.getAllResults().size() > 0) {
            ...
            int numberOfMethods = getMethodSet(tests, suite).size();
            ITestNGMethod[] methods = new ITestNGMethod[numberOfMethods];
            methods = getMethodSet(tests, suite).toArray(methods);
            for (methodIndex=0;methodIndex<numberOfMethods;methodIndex++) {
                m_row += 1;
                m_methodIndex += 1;
    ...
    private void resultDetail(IResultMap tests) {
        ...
        for (ITestResult result : testResultsList) {
            result = testResultsList.get(index);
            ITestNGMethod method = result.getMethod();
            m_methodIndex++;
            String cname = method.getTestClass().getName();
            writer.println("<h2 id=\"m" + m_methodIndex + "\">" + cname + ":"
                    + method.getMethodName().toString() + "</h2>");
            Set<ITestResult> resultSet = tests.getResults(method);
    ...
    private class TestSuiteComparator implements Comparator<ISuite> {
